# Multi tenancy bot — Style Reference
> golden hour editorial dashboard

**Theme:** light

Multi Tenancy Bot sits at the intersection of conversational AI and editorial software UI — a warm parchment canvas (never pure white) frames deep‑maroon brand moments and vivid violet accents. Headlines whisper at weight 460 rather than shout, letting the cinematic full‑bleed hero photograph of human‑agent collaboration do the emotional heavy lifting while translucent product UI cards float like glass over the image. The palette stays quiet and editorial most of the time: cream surfaces, charcoal text, hairline warm‑gray borders, and generous spacing, with color reserved for deliberate brand punctuation — a single maroon CTA to onboard a tenant, violet links for documentation, a light‑lilac secondary button for support. The system deliberately avoids heavy shadows and gradients on UI surfaces; depth comes from layered photography of advisors at work and subtle backdrop blur on the header, not from elevation stacks.

## Design Principles

1. **Editorial warmth over UI chill** — Use cream parchment (#f2f0eb) as your base canvas instead of pure white. Apply warm-gray hairline borders (#e3e3e2) instead of heavy drop shadows. Let generous white-space create a sense of calm and focus.

2. **Quiet authority at headline level** — Reserve weight 460 for your main section titles (48px, weight 460, charcoal #292827). It reads as confident and considered, not thin or unfinished.

3. **Color as punctuation, not decoration** — Use deep maroon (#421d24) only for high-priority CTAs (onboard a tenant, start free trial). Use violet (#714cb6) exclusively for links and short semantic highlights. Keep everything else neutral.

4. **Translucent layers over heavy shadows** — Use backdrop-filter blur(12px) for the navigation header so it floats subtly over the hero image. Avoid applying shadows directly to cards or buttons; rely instead on the warm background and hairline borders for definition.

5. **Photography-forward layout** — Treat full-bleed hero and section-break images as primary design elements, not glorified stock photos. Use them as environments where UI components (cards, chips, short text blocks) can float with intentionality.

6. **Purpose-driven architecture** — Treat the Multi Tenancy Bot as a system of layers rather than a monolithic web application. Use Next.js for the portal (Next.js API Routes for tenants.json operations, Next.js Server Components for dashboard pages, route handlers for webhooks) and Fast API for the RAG runtime (FastAPI routes for /webhook and /admin). Keep the two surfaces intentionally separate: the portal is the human control plane, the RAG runtime is the message-processing engine.

7. **Data-first multi-tenancy** — Keep tenant configuration, API keys, and session metadata in tenants.json. Ensure all tenant-specific logic (routing webhooks, selecting knowledge collections, setting conversation boundaries) reads from this file at runtime. Do not embed secrets or tenant IDs in client-side code.

8. **Operational transparency** — Expose an /admin health endpoint that reports Redis connectivity, ChromaDB collection status, and the currently configured `ADVISOR_NOTIFY_WHATSAPP`. Use this endpoint in your monitoring stack. For debugging, provide a /admin/debug/conversations/{phone_number} route that shows recent messages and retrieval context (truncated for privacy).

9. **Resilience through redundancy** — Implement a Redis failover strategy: if the primary Redis instance is unavailable, automatically route new sessions to a backup Redis instance or gracefully degrade to local file-based sessions with a clear warning banner in the admin UI.

10. **Security by isolation** — Apply strict request validation and rate limiting on the /webhook endpoint to prevent abuse. Ensure API keys and JWT secrets are stored as environment variables only and never committed to version control.
...
Background uses a layered composition of radial gradients: violet glow at 68%/50%, blue glow at 93%/50%, pink glow at 50%/98%, cyan glow at 50%/75%, all at 50-60% opacity. Headline #292827 at 48px/460 left-aligned. White "Get Multi tenancy bot" button right-aligned. The gradients are decorative atmosphere, not interactive surfaces.
...
### Section Heading
**Role:** Display-level headlines for major sections ("Your Multi tenancy bot suite", "The most productive…")
...
- **Linear** — Same weight-460-at-large-sizes headline approach and preference for hairline borders over heavy shadows, though Linear is dark-first and Multi tenancy bot is light-first
